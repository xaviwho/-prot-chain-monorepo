...
const nextConfig = {
  ...
  async rewrites() {
    return [
      ...
      // Proxy workflows API calls to AWS backend
      {
        source: '/api/v1/workflows',
        destination: `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8082'}/api/v1/workflows`,
      },
      {
        source: '/api/v1/workflows/:path*',
        destination: `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8082'}/api/v1/workflows/:path*`,
      },
